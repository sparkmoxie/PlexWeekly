...
function Assert-RendererContract([string]$PackageName, [string]$Renderer) {
    ...
    Assert-True ($Renderer.Contains('function Test-TautulliLocalUserId') -and $Renderer.Contains('Tautulli Local user ID 0 is not a newsletter recipient.')) "$PackageName lacks the reserved Local recipient guard."
    Assert-True ($Renderer.Contains('$directId -eq $Id')) "$PackageName does not verify the requested Tautulli lookup identity."
    Assert-True ($Renderer.Contains('if (Test-TautulliLocalUserId -Id ([string]$User.UserId)) { return "excludedUserId" }')) "$PackageName lets a fallback assignment bypass the reserved Local exclusion."
...
foreach ($asset in $gifManifest.assets) {
    $expectedGifHashes[$asset.name] = $asset.sha256.ToUpperInvariant()
}
Assert-True ($expectedGifHashes.Count -eq 32) 'Expected all 32 optimized GIFs in every release archive.'
foreach ($genreName in $expectedGenreHashes.Keys) {
    Assert-True ($expectedGifHashes[$genreName] -ceq $expectedGenreHashes[$genreName]) "Genre manifests disagree: $genreName"
}
