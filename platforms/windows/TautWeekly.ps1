...
$script:TautWeeklyResultErrorCategory = "operation-busy"
...
if (-not (Test-Path $ConfigPath)) {
    throw "Config file not found: $ConfigPath`nOpen TautWeekly Manager and complete Config, or run 00-SETUP-FIRST.bat as a terminal fallback."
}
...
function Get-LatestReleaseData {
    ...
    foreach ($sectionId in @(Get-LatestReleaseQueryScopes)) {
        ...
        for ($page = 0; $page -lt $maxPages; $page++) {
            ...
            $enoughForScope = if ($script:LibraryFilterEnabled -or $isSectionScope) {
...
function Get-DesignPlexContext {
                ...
                foreach ($line in (Get-Content -Path $candidatePath -Encoding UTF8 -ErrorAction Stop)) {
                    if ($line -match '^\s*pms_token\s*=\s*(.+?)\s*$') {
                        ...
                        if (-not [string]::IsNullOrWhiteSpace($candidateToken) -and
                            ...
                            Write-Log ("TautWeekly for Plex: Plex token loaded from Tautulli config: " + $candidatePath)
                            break
                        }
                    }
                }
    ...
    if ([string]::IsNullOrWhiteSpace($token)) {
        Write-Log "TautWeekly for Plex: Plex token unavailable from environment, Plex registry, or discovered Tautulli configs." "WARN"
    }
...
function Test-TautWeeklyDirectPlexConnection {
    ...
    try {
        ...
        Write-Log ("Direct Plex verification failed ({0}). Confirm the private PlexServerUrl and PlexToken are valid and reachable from this runtime." -f $safeFailure) "ERROR"
        return 2
    }
}
...
if ($Mode -eq "VerifyPlex") {
    $script:TautWeeklyResultErrorCategory = "plex-unavailable"
    $verifyExitCode = Test-TautWeeklyDirectPlexConnection
    Write-TautWeeklyStructuredResult -Outcome $(if ($verifyExitCode -eq 0) { "succeeded" } else { "failed" })
    exit $verifyExitCode
}
